...
namespace Trinity
{
    ...
    namespace {
        ...
        string json_unescape(const string& str) {
            ...
            boost::replace_all(output, "\\\\";'\\');
            boost::replace_all(output, "\\b", '\b');
            boost::replace_all(output, "\\f", '\f');
            boost::replace_all(output, "\\n", '\n');
            boost::replace_all(output, "\\r", '\r');
            boost::replace_all(output, "\\t", '\t');
            return std::move(output);              
        }
        */
    ...
    public:
        ...
        string ToUnescapedString() const { return (Type == Class::String ? std::move(*Internal.String) : string("")); }
        ...
        string dump(bool minify = true, int depth = 1, string tab = "  ") const {
            string pad = "";
            string charNewLine = (!minify ? "\n" : "");
            tab = (!minify ? tab : "");
            ...
            case Class::Null:
                return "null";
            case Class::Object: {
                string s = "{" + charNewLine;
                bool skip = true;
                for (auto& p : *Internal.Map) {
                    if (!skip) s += "," + charNewLine;
                    s += (pad + "\"" + p.first + "\": " + p.second.dump(minify, depth + 1, tab));
                    skip = false;
                }
                s += (charNewLine + pad.erase(0, 2) + "}");
                return s;
            }
            case Class::Array: {
                string s = "[";
