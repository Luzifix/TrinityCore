...
#define BnetGameAccountInfo "a.id, a.username, ab.unbandate, ab.unbandate = ab.bandate, aa.SecurityLevel"
    ...
    PrepareStatement(LOGIN_SEL_BNET_SIMILAR_HARDWARE_HISTORY_ENTRIES, "SELECT id, CAST(IF(h.macHash = ? , 1, 0) AS INT) AS `macHashMatch`, CAST(IF(h.gatewayMacHash = ? , 1, 0) AS INT) AS `gatewayHashMatch`, CAST(IF(h.machineHash = ? , 1, 0) AS INT) AS `machineeHashMatch` FROM battlenet_accounts_hardware_history h WHERE id != ? AND(h.macHash = ? OR h.gatewayMacHash = ? OR h.machineHash = ? ) HAVING(macHashMatch = 1 AND id NOT IN(SELECT w1.id FROM battlenet_accounts_hardware_whitelist w1 WHERE w1.`macHash` = h.macHash)) OR(gatewayHashMatch = 1 AND id NOT IN(SELECT w2.id FROM battlenet_accounts_hardware_whitelist w2 WHERE w2.`gatewayMacHash` = h.gatewayMacHash)) OR(machineeHashMatch = 1 AND id NOT IN(SELECT w4.id FROM battlenet_accounts_hardware_whitelist w4 WHERE w4.`machineHash` = h.machineHash)) ORDER BY `machineeHashMatch` DESC, `gatewayHashMatch` DESC, `macHashMatch` DESC;", CONNECTION_BOTH);
