...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomBonusListId, " \
    ...
    PrepareStatement(CHAR_SEL_MAIL_SENDER_LIST_UNREADED, "SELECT c.name FROM mail m INNER JOIN characters c ON (c.guid = m.sender) WHERE checked & 1 = 0 AND receiver = ? GROUP BY c.name ORDER BY m.id DESC;", CONNECTION_SYNCH);
