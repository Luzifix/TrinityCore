...
{
    data.WriteBit(displayInfo.FileDataID.has_value());
...
WorldPacket const* WorldPackets::BattlePay::SyncWowEntitlements::Write()
{
    ...
    for (auto const& product : Product)
    {
        ...
        _worldPacket.WriteBit(product.DisplayInfo.has_value());
        ...
        for (auto productItem : product.Items)
        {
            ...
            _worldPacket.WriteBit(productItem.DisplayInfo.has_value());
